<x-web-layout>

    <section class="page-header">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h1>Ahimsa</h1>
                    <ul class="breadcrumb breadcrumb--classic">
                        <li><a href="/"><i class="fa fa-home"></i></a></li>
                        <li class="active"><a>Ahimsa</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </section>

    <section class="details mb-3">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <p>Ahimsa is the Alchemist of flexible thinking. It will give you  a confident, respectful demeanor while solving your problems constructively. Ahimsa <i>transforms competition into collaboration</i>.</p>
                    <div class="row">
                        <div class="col-md-4 mb-3 p-5">
                            <img src="{{ asset('images/web/alchemists/one.jpg') }}" alt="" width="100%" height="100%">
                        </div>
                        <div class="col-md-6 mb-3 p-5">
                            <h2>One with Ahimsa</h2>
                            <h3>'One'</h3>
                            <p>- An Acrylic on canvas (12” radius) knife painting by Dr. Shaifali Gupta. It  is inspired by the principle of Oneness from which we need to operate for practicing Ahimsa.</p>
                        </div>
                    </div>
                    <p><i>Ahimsa </i>is the Alchemist who calms us and gives us direction when we are angry.</p>
                    <p><i>Ahimsa, </i>the first Yama, invites us to stop any blame games, and take self-responsibility for our own happiness. Psychology too emphasises self-responsibility as the first step to positivity.</p>
                    <p><i>Ahimsa </i>will help you solve our conflicts better by being more flexible and empathetic.</p>
                    <div class="row how">
                        <div class="col-md-3 mb-3 p-5">
                            <img src="{{ asset('images/web/illustrations/bee2.jpg') }}" alt="" width="100%" height="100%">
                        </div>
                        <div class="col-md-9 mb-3 p-5">
                            <p><i>'When I feel angry, all I actually need is a big hug from you', said the Friendly Flower.</i></p>
                        </div>
                    </div>
                    <h2>Elements of Ahimsa</h2>
                    <div class="row how">
                        <div class="col-md-6 mb-3 p-5 text-center">
                            <p><b>Ahimsa<br/>=<br/>Empathy<br/>+<br/>Flexibility<br/>+<br/>Self-responsibility<br/>+<br/>Courage<br/><i class="fa fa-long-arrow-down"></i><br/>Constructive problem solving</b></p>
                        </div>
                        <div class="col-md-6 mb-3">
                            <img src="{{ asset('images/web/elements/element1.jpg') }}" alt="">
                        </div>
                    </div>
                    <h3>Are you already friends with Ahimsa?</h3>
                    <p>Take a self-reporting psychological assessment.</p>
                    <h4>Assertiveness assessment</h4>
                    <p>Being assertive gives us a confident, respectful demeanor that allows you to communicate effectively with others. Practicing assertive communication helps improve our interactions which would result in more effective team building, leadership and collaboration. Go through the assertiveness assessment to know yourself better.</p>
                    <p class="principal"><a href="{{ route('assertivenessAssessment') }}">Assess</a> your conflict resolution style.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="contact">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="text-center">Contact Us</h2>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    <form class="contact-form" method="post" action="{{ route('sendMessage') }}">
                        <div class="form-group mb-3">
                            <input type="text" class="form-control" id="name" name="name" placeholder="Enter Full Name" oncopy="return false" onpaste="return false" required>
                        </div>
                        <div class="form-group mb-3">
                            <input type="email" class="form-control" id="email" name="email" placeholder="Enter Email" oncopy="return false" onpaste="return false" oninvalid="this.setCustomValidity('Please enter a valid email')" oninput="this.setCustomValidity('')" required>
                        </div>
                        <div class="form-group mb-3">
                            <input type="text" class="form-control" id="mobile" name="mobile" placeholder="Enter Mobile Number" minlength="10" maxlength="10" oncopy="return false" onpaste="return false" oninvalid="this.setCustomValidity('Please enter a valid mobile number')" oninput="this.setCustomValidity('')" pattern="[6-9]{1}[0-9]{9}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required>
                        </div>
                        <div class="form-group mb-3">
                            <textarea type="text" class="form-control" id="msg" name="msg" placeholder="Enter Your Message" rows="5" oncopy="return false" onpaste="return false"></textarea>
                        </div>
                        <div class="form-group text-center mb-3">
                            <input type="submit" class="btn btn-send" id="send" name="send" value="Send">
                        </div>
                    </form>
                </div>
                <div class="col-md-6 mb-3">
                    <div class="contact-image">
                        <img src="{{ asset('images/web/contact-image.jpg') }}" alt="" width="100%" height="100%">
                    </div>
                </div>
            </div>
        </div>
    </section>

</x-web-layout>